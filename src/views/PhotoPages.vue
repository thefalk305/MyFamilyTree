...
for (var i = 0; i < photos.value.length; i++) {
  for (var j = 0; j < infoTable.value.length; j++) {
    if (photos.value[i] === infoTable.value[j].name) {
...
//   }
//   items = response.data;
//   // wholeArray.value = items;
//   // copy info from db table for five brothers
//   for (var i = 0; i < photos.value.length; i++) {
//     for (var j = 0; j < items.length; j++) {
//       if (photos.value[i] === items[j].name) {
//         recordData.value.push(items[j]);
//         break;
//       }
//     }
//   }
...
<template>
  <div id="photo-pages">
    ...
    <div id="content">
      <div id="photoGallery">
        <div id="Photo" v-for="photo in recordData">
          <p id="photoName">
            <!-- {{ Photo.name }} -->

...
infoTable.forEach((record) => {
  ...
  for (var i = 0; i < photos.value.length; i++) {
    if (pic === photos.value[i].pic) duplicate = true;
  }
  if (!duplicate) photos.value.push(record);
});
  ...
  <div id="photo-pages">
    ...
    <div id="flex-container">
      <div id="leftnav">
        <b>Picture Pages</b>
        <ul class="u-list">
          <li v-for="{ route, text, index } in ulList">
            <AppLink :to="route">{{ text }}</AppLink>
          </li>
        </ul>
        <b>Places</b>
        <ul class="u-list">
          <li v-for="{ route, text } in llList">
            <AppLink :to="route">{{ text }}</AppLink>
          </li>
        </ul>
      </div>
      <div id="rightnav">
        <b>Transportation</b>
        <ul class="u-list">
          <li v-for="{ route, text } in urList">
            <AppLink :to="route">{{ text }}</AppLink>
          </li>
        </ul>
        <b>Other</b>
        <ul class="u-list">
          <li v-for="{ route, text } in lrList">
            <AppLink :to="route">{{ text }}</AppLink>
          </li>
        </ul>
      </div>
      <div id="content">
        <!-- <div> -->
        <button class="center" @click="deblurAll">Deburr All</button>
        <div id="photoGallery">
          <div class="photos" :id="index" v-for="(photo, index) in photos">
            <p id="photoName">
              <!-- {{ photo.name }} -->
              <img
                :id="'photo' + index"

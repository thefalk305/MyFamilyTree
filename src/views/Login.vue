...
infoTable.value = inject("infoTable");
    ...
    function login() {
      // getRecord();
      for (var i = 0; i < infoTable.value.length; i++) {
        if (username.value === infoTable.value[i].username) {
          userPw.value = infoTable.value[i].password;
          record.value = infoTable.value[i];
          break;
        }
      }
